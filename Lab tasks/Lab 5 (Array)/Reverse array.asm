	;take integer numbers from user using an array. then print all values in reverse order :
    ...
    ; we can use mov si, 4  but then the index value will become static(fixed) we need it to be dynammic (user input)
    ;that's why we use bx =bh+bl which will have the index value beacause bl has the index value
	
	mov bh, 0   ; to make bx register = 16 bit, as si cannot take any data from 8 bit register, now bl+bh = bx, 
	             ;whole 16 bit holding the size
	...
	reverse:
			...
			mov ah, 2
			mov dl, array[si]
			add dl, 48        ; now the value became declared in the program so we ,  add 48 for decimal convertion
			int 21h
